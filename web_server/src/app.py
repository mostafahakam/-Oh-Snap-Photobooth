...
def detect_faces_in_image(file_stream):
    ...
    if len(unknown_face_encodings) > 0:
        ...
        for row in Row.select():
            ...
            np = np.frombuffer(curr_encoding, dtype=unknown_face_encodings[0].dtype).reshape(unknown_face_encodings[0].shape)

            match_results = face_recognition.compare_faces(curr_encoding, unknown_face_encodings[0])
            if match_results[0]:
                result = row.user_id
                break
